...
private:
  bool start_cap();
  bool stop_cap();
  bool service_toggle_cap(std_srvs::SetBool::Request &, std_srvs::SetBool::Response &);
  void spin();
  bool take_and_send_image();
  void load_from_parameters(ros::NodeHandle &);
  void recalibrate_camera_info();
  void set_v4l_params();
...
void UsbCamNodelet::onInit() {
  ...
  service_toggle_capture_ =
      pnh.advertiseService("toggle_capture", &UsbCamNodelet::service_toggle_cap, this);
}
...
bool UsbCamNodelet::start_cap() {
  std::unique_lock<std::mutex> cam_lock(cam_mutex_);
  if (!cam_.is_capturing()) {
    // if capturing was recently stopped, we should wait for thread to finish
    // execution
    cam_.start(video_device_name_.c_str(), io_method_, pixel_format_, image_width_, image_height_,
               framerate_);
    set_v4l_params();
    cam_lock.unlock();
    try {
...
bool UsbCamNodelet::service_toggle_cap(std_srvs::SetBool::Request &req,
...
void UsbCamNodelet::spin() {
  if (io_method_ == UsbCam::IO_METHOD_UNKNOWN || pixel_format_ == UsbCam::PIXEL_FORMAT_UNKNOWN) {
    NODELET_ERROR_STREAM("Unknown IO method " << io_method_name_ << " or Pixel format "
                                              << pixel_format_name_);
    return;
  }
...
void UsbCamNodelet::load_from_parameters(ros::NodeHandle &nh) {
...
void UsbCamNodelet::recalibrate_camera_info() {
...
void UsbCamNodelet::set_v4l_params() {
  if (brightness_ >= 0) {
